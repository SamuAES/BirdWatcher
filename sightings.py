...
def get_all_sightings(user_id = None):
    if user_id is None:
        sql = """SELECT S.id, U.username, S.bird_name, S.time, S.location, S.additional_info FROM Users U, Sightings S 
                WHERE U.id = S.user_id AND S.visibility = true ORDER BY S.time DESC"""
        result = db.session.execute(text(sql))
    else:
        sql = """SELECT S.id, U.username, S.bird_name, S.time, S.location, S.additional_info FROM Users U, Sightings S 
                WHERE U.id = S.user_id AND U.id = :user_id AND S.visibility = true ORDER BY S.time DESC"""
        result = db.session.execute(text(sql), {"user_id":user_id})
    return result.fetchall()

def get_sightings_by_name(birdname):
    sql = """SELECT S.id, U.username, S.bird_name, S.time, S.location, S.additional_info FROM Users U, Sightings S 
            WHERE U.id = S.user_id AND S.bird_name = :birdname AND S.visibility = true ORDER BY S.time DESC"""
...
def get_follows_sightings(follows_id_numbers):
    try:
        sql = """SELECT S.id, U.username, S.bird_name, S.time, S.location, S.additional_info FROM Users U, Sightings S 
                WHERE U.id = S.user_id AND U.id IN :follows_id_numbers AND S.visibility = true ORDER BY S.time DESC"""
...
def get_sighting_details(id):
    sql = """SELECT U.username, S.bird_name, S.time, S.location, S.additional_info FROM Users U, Sightings S 
...
def new_sighting(bird_name, time, location, additional_info, image):
    ...
    sql = """SELECT id FROM Sightings WHERE user_id = :user_id AND bird_name = :bird_name AND time = :time 
